...
This document explains only Tenutino's Chatbot and AI Tutor system. Character movement, score-following animation, parkour, dragging, and the A/B Listening Challenge are outside its primary scope except where they connect to the Chatbot.

本文档只解释 Tenutino 的 Chatbot／AI Tutor 系统。角色移动、乐谱跟随动画、Parkour、拖拽，以及 A/B Listening Challenge 不属于本文的主要范围；只有在它们与 Chatbot 发生连接时才会被提及。
...
This mode currently returns text. It does not mutate the progression or create an executable edit. Executable comparisons belong to the separate Active Learning system.
...
Short A/B lesson playback is managed separately by the Active Learning flow.
...
- Does not read the cross-project learner profile when choosing vocabulary or difficulty.
...
- Does not yet pass both A/B lesson voicings as structured comparison evidence to GPT.

The final point matters: `Explain why` can explain the current seam accurately, but a fully grounded explanation comparing the original and Version B requires both voicings and their computed differences to be added to the coach payload.
...
当前 Suggestions 只返回文字，不会修改 progression，也不会自动创建可执行编辑。可执行的音乐对比属于独立的 Active Learning 系统。
...
Active Learning 的短 A/B 播放由另一条流程控制。
...
- 不读取跨项目 learner profile 来调整词汇和难度。
- Suggestions 仍然是文字，不是可直接执行的编辑。
- 不使用流式输出，需要等待完整响应。
- 只计算共同音和外声部运动等选定证据，没有完整的内声部配对。
- 尚未把 A/B Lesson 的两个 voicing 都作为结构化比较证据传给 GPT。

最后一点尤其重要：`Explain why` 可以准确解释当前 seam，但如果要完整、可信地比较 Original 与 Version B，还需要把两个 voicing 以及它们之间的确定性差异加入 Coach payload。
